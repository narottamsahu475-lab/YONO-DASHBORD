...
# --- BUG 2 FIXED: Added registration_summary node to central stats matrix ---
live_stats = {
...
async def production_health_checker_loop():
    ...
    while True:
        try:
            ...
            try: r_sim = await loop.run_in_executor(None, lambda: requests.get(f"https://api.4sim.st/getBalanceAndRating?apikey={FOUR_SIM_API_KEY}", timeout=5)); s_st = "Connected" if r_sim.status_code in [200, 400, 401] else "Offline"
            except Exception: s_st = "Offline"
            try: r_pan = await loop.run_in_executor(None, lambda: requests.get(BASE_URL, headers=HEADERS, timeout=5)); p_st = "Connected" if r_pan.status_code in [200, 401, 403, 404, 405] else "Offline"
...
async def add_activity(phone, text, color_val="🟢"):
    async with stats_lock:
        live_stats["recent_activity"].insert(0, {"phone": phone, "current_game": "System Engine", "status": text, "balance": "0 INR", "retry": 1, "progress": 100, "thread_color": color_val})
...
# BUG 2 FIXED: Added registration summary incremental logic pipeline
...
# BUG 1 FIXED: Augmented update_live_status to accept and dynamically map current_game, retry, progress, and color nodes
...
async def run_game_step_async(phone, txn_id, app_name, used_otps, is_sub_game=False, active_wallet_ref="0 INR"):
    ...
    while send_attempt <= 3:
        try:
            # BUG 1 FIXED: Dynamically streams app_name and live progress percentages cleanly down to worker panel cards
...
async def process_single_registration():
    ...
    try:
        ...
        async with buy_lock:
            ...
            try:
                async with http_session.get(buy_url, timeout=12) as response:
                    ...
                    except Exception: logger.error("JSON parse error."); return
        ...
        # BUG 1 FIXED: Initialized recent_activity arrays structure with complete matching schema fields
        async with stats_lock:
        ...
        if main_res == "already":
            ...
            # BUG 3 FIXED: Save profiles metrics inside precise structural formats matching layout specifications
            elapsed = round(time.time() - worker_start_time, 1)
            async with stats_lock:
                live_stats["success_records"].insert(0, {
                    "phone": phone,
                    "games_registered": games_registered,
