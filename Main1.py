...
FOUR_SIM_API_KEY = os.getenv("12b75fd7d823107676094a9aa8cd3ccc")
PANTHERS_API_KEY = os.getenv("panthers_59T9iL6hei22g7W_uPij_k56ncRBG8nEW8mY6Q")
BASE_URL = os.getenv("PANTHERS_BASE_URL", "https://panthers.accbazaar.shop")
HEADERS = {"X-API-Key": PANTHERS_API_KEY, "Content-Type": "application/json"}
http_session, health_task = None, None
...
def mask_sensitive(d): return f"{str(d)[:3]}****{str(d)[-3:]}" if len(str(d)) > 6 else "****"
...
async def production_health_checker_loop():
    ...
    while True:
        try:
            try: r_net = await loop.run_in_executor(None, lambda: requests.get("https://1.1.1.1", timeout=3)); n_st = "Connected" if r_net.status_code == 200 else "Offline"
            except Exception as e: n_st = "Offline"; logger.debug(f"Net check fail: {e}")
            try: r_sim = await loop.run_in_executor(None, lambda: requests.get(f"https://api.4sim.st/getBalance?apikey={FOUR_SIM_API_KEY}", timeout=4)); s_st = "Connected" if r_sim.status_code == 200 else "Offline"
            except Exception as e: s_st = "Offline"; logger.debug(f"Sim check fail: {e}")
            try: r_pan = await loop.run_in_executor(None, lambda: requests.get(BASE_URL, timeout=4)); p_st = "Connected" if r_pan.status_code in [200, 404, 405] else "Offline"
            except Exception as e: p_st = "Offline"; logger.debug(f"Panther check fail: {e}")
            async with stats_lock: live_stats["health_check"] = {"internet": n_st, "4sim": s_st, "panther": p_st}
        except asyncio.CancelledError: break
        except Exception as e: logger.error(f"Health fault tracker: {e}")
        await asyncio.sleep(30)

@app.on_event("startup")
async def app_startup_handler():
    global http_session, health_task
    if not FOUR_SIM_API_KEY or not PANTHERS_API_KEY: logger.critical("CRITICAL: Production Environment Keys Missing!")
    http_session = aiohttp.ClientSession()
    health_task = asyncio.create_task(production_health_checker_loop())
...
async def add_activity(phone, text):
    async with stats_lock:
        live_stats["recent_activity"].insert(0, {"phone": phone, "current_game": "System", "status": text, "balance": "0 INR", "retry": 1, "progress": 100, "thread_color": "🔴"})
        if len(live_stats["recent_activity"]) > 200: live_stats["recent_activity"].pop()
...
async def add_timeline_event(phone, stage, current_game="System"):
    async with stats_lock:
        ...
        if len(live_stats["activity_timeline"]) > 500: live_stats["activity_timeline"].pop()
...
async def fetch_smart_otp_async(txn_id, used_otps):
    ...
    try:
        async with http_session.get(url, timeout=5) as resp:
            ...
            if sms_text:
                all_codes = re.findall(r'\d{4}|\d{6}', sms_text)
                new_codes = [c for c in all_codes if c not in used_otps]
                if new_codes: return new_codes[-1]
    except Exception as e: logger.error(f"OTP Fetch err profile: {e}")
    return None
...
async def terminate_4sim_order_async(txn_id, otp_received, phone, force_cancel=False):
    ...
    for attempt in range(1, 4):
        ...
        except Exception as e: logger.debug(f"Close retry pass context err: {e}")
...
async def handle_already_number(phone, txn_id, otp_flag):
    ...
    except Exception as e: logger.error(f"Hold executor fail context track: {e}")
    finally:
...
async def handle_failed_number(phone, txn_id):
    ...
    except Exception as e: logger.error(f"Failure worker process fault: {e}")
...
async def run_game_step_async(phone, txn_id, app_name, used_otps, is_sub_game=False, active_wallet_ref="0 INR"):
    ...
    while send_attempt <= 3:
        try:
            await update_live_status(phone, app_name, "Running Request", active_wallet_ref, send_attempt, 5, "🟢")
            await add_timeline_event(mask_sensitive(phone), f"Call chunk payload (Attempt {send_attempt})", app_name)
            async with http_session.post(f"{BASE_URL}/v1/register/send_otp", headers=HEADERS, json={"phone": phone, "app_name": str(app_name)}, timeout=12) as response:
                if not (200 <= response.status < 300): send_attempt += 1; await asyncio.sleep(4); continue
                try: v_res = await response.json(content_type=None)
                except Exception as e: logger.debug(f"JSON load failure: {e}"); send_attempt += 1; await asyncio.sleep(4); continue
                ...
                if v_res.get("status") == "success":
                    ...
                    if not task_id: send_attempt += 1; await asyncio.sleep(4); continue
                    ...
                    for elapsed in range(0, 131, 5):
                        ...
                        if otp:
                            ...
                            async with http_session.post(f"{BASE_URL}/v1/register/verify_otp", headers=HEADERS, json={"task_id": str(task_id), "otp": str(otp)}, timeout=15) as v_resp:
                                if not (200 <= v_resp.status < 300): return "failed", active_wallet_ref
                                try: verify_res = await v_resp.json(content_type=None)
            ...
            send_attempt += 1; await asyncio.sleep(4)
        except Exception as ex: logger.error(f"Step exception: {ex}"); send_attempt += 1; await asyncio.sleep(4)
        ...
        async with buy_lock:
            ...
            try:
                async with http_session.get(buy_url, timeout=12) as response:
                    ...
                    try: buy_res = await response.json(content_type=None)
                    except Exception as e: logger.error(f"JSON decoder fault: {e}"); return
                    phone = str(buy_res.get("number", ""))[-10:]; txn_id = buy_res.get("tid") or buy_res.get("id")
                    if not phone: return
                    async with stats_lock: success_buy_count += 1; active_task_counter += 1; live_stats["total_secured"] = success_buy_count
            except Exception as e: logger.error(f"Stock procurement crash: {e}"); return
        async with stats_lock:
            live_stats["recent_activity"].insert(0, {"phone": phone, "current_game": "Bingo_game", "status": "Running Request", "balance": last_balance, "retry": 1, "progress": 0, "thread_color": "🟢"})
            if len(live_stats["recent_activity"]) > 200: live_stats["recent_activity"].pop()
        main_res, last_balance = await run_game_step_async(phone, txn_id, "Bingo_game", used_otps, False, last_balance)
        if main_res == "already":
            ...
            games_registered = 1; await asyncio.sleep(2)
            other_games = ["yonoslot_game", "spincrush_game", "567slot_game", "789jackpot_game", "mbmbet_game", "hirummy_game", "Yonogame_game", "maha_game"]
            ...
            async with stats_lock:
                ...
                if len(live_stats["success_records"]) > 300: live_stats["success_records"].pop()
            await terminate_4sim_order_async(txn_id, True, phone)
        await increment_completed_metric()
    except Exception as exc: logger.exception(f"Primary process runner crashed: {exc}")
    finally:
