...
async def handle_already_number(phone, txn_id, otp_flag):
    ...
    try:
        ...
        active_already_tasks.discard(asyncio.current_task())
        await remove_worker_node(phone)
...
async def run_game_step_async(phone, txn_id, app_name, used_otps, is_sub_game=False, active_wallet_ref="0 INR"):
    ...
    while send_attempt <= 3:
        try:
            ...
            await add_timeline_event(mask_sensitive(phone), f"Call payload (Attempt {send_attempt})", app_name)
            async with http_session.post(f"{BASE_URL}/v1/register/send_otp", headers=HEADERS, json={"phone": phone, "app_name": str(app_name)}, timeout=12) as response:
                ...
                # --- LIVE DEBUG DUMP INJECTED CLEANLY HERE ---
                print("====================================")
                print("Panther Status (Send OTP):", response.status)
                print("Panther Response (Send OTP):", raw_t)
                print("====================================")
                
                if response.status != 200: send_attempt += 1; await asyncio.sleep(4); continue
                try: v_res = json.loads(raw_t)
                except Exception: send_attempt += 1; await asyncio.sleep(4); continue
                    ...
                    for elapsed in range(0, 131, 5):
                        ...
                        if otp:
                            ...
                            async with http_session.post(f"{BASE_URL}/v1/register/verify_otp", headers=HEADERS, json={"task_id": str(task_id), "otp": str(otp)}, timeout=15) as v_resp:
                                ...
                                # --- EXTRA VERIFY FLOW TELEMETRY DUMP ---
                                print("====================================")
                                print("Panther Status (Verify OTP):", v_resp.status)
                                print("Panther Response (Verify OTP):", raw_v)
                                print("====================================")
        ...
        except Exception: send_attempt += 1; await asyncio.sleep(4)
